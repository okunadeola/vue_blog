...
<template>
  <div
    class="group rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow cursor-pointer h-full relative"
    @click="viewDetail(post?.slug)"
  >
    <img
      :src="post?.image"
      :alt="post?.title"
      class="w-full h-48 bg-red-500 object-cover transition-transform duration-500 group-hover:scale-105"
    />
    <div
